        ...
        await AppDataSource.transaction(async (manager) => {
            ...
            for (const input of AUTOMOTIVE_MOCK_PROVIDERS) {
                ...
                const provider = await providerRepository.save(providerRepository.create({
                    ...
                    phone: input.phone ?? input.phones?.[0] ?? null,
                    phones: [...(input.phones ?? (input.phone ? [input.phone] : []))],
                    email: input.email ?? null,
                    communicationMode: input.communicationMode ?? 'online',
                    chatEnabled: input.chatEnabled ?? input.communicationMode !== 'phone_only',
                    responseWindowMinutes: input.communicationMode === 'phone_only' ? null : input.responseWindowMinutes ?? 240,
