...
export class HardenAutoCareAttachmentIntegrity1786280000000 implements MigrationInterface {
    ...
    async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "autocare_service_attachments" ADD CONSTRAINT "CHK_autocare_attachments_parent" CHECK (("requestId" IS NOT NULL AND split_part("objectKey", '/', 1) = 'autocare-requests' AND split_part("objectKey", '/', 2) = "requestId"::text) OR ("threadId" IS NOT NULL AND split_part("objectKey", '/', 1) = 'autocare-chats' AND split_part("objectKey", '/', 2) = "threadId"::text)) NOT VALID`)
