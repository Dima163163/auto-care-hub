...
export type QualityProvider = { id: string; status: string }
...
export type QualityMessage = { requestId: string | null; senderId: string; createdAt: Date }
...
export function buildQualityMetrics(input: {
    ...
    const requestClientById = new Map(input.requests.map((request) => [request.id, request.providerId]))
    for (const message of input.messages) {
        if (!message.requestId || firstProviderMessage.has(message.requestId)) continue
        // The caller supplies only provider-authored messages for this metric.
        if (requestClientById.has(message.requestId)) firstProviderMessage.set(message.requestId, message.createdAt)
    }
