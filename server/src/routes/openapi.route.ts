    ...
    return {
        ...
        paths: {
            ...
            '/owner/service-requests/{requestId}/reschedule': {
                post: { operationId: 'requestAutoCareServiceReschedule', parameters: [{ name: 'requestId', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['proposedAt'], properties: { proposedAt: { type: 'string', format: 'date-time' }, reason: { type: ['string', 'null'], maxLength: 1_000 } }, additionalProperties: false } } } }, responses: { '200': { description: 'Provider proposes a new visit time.' } } },
            },
