...
describe('password reset email', () => {
    it('creates a frontend URL with an encoded token', () => {
        expect(
            ...
            'https://app.example.com/password/reset?token=token+with+reserved%3Fcharacters'
        )
    })

    it('includes the reset link and expiry in text and HTML', () => {
        const message = createPasswordResetEmail(input)

        expect(message.to).toBe(input.email)
        expect(message.text).toContain(
            'https://app.example.com/password/reset?token=one-time-reset-token'
        )
    ...
    it('sends the generated message through the mailer abstraction', async () => {
        ...
        await sendPasswordResetEmail({ send }, input)
