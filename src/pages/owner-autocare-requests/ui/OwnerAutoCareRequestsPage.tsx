...
import { formatCurrency, formatDateTime, formatDateTimeInTimeZone } from '@/shared/lib/locale-format'
...
function SummaryCard({ icon: Icon, label, value, loading = false }: { icon: typeof Clock3; label: string; value: number; loading?: boolean }) {
    return <div className="rounded-[var(--radius-panel)] border border-border bg-card p-4 shadow-sm" aria-busy={loading}><Icon className="size-5 text-primary" />{loading ? <Skeleton className="mt-3 h-8 w-12" aria-label={label} /> : <p className="mt-3 text-2xl font-black text-foreground">{value}</p>}<p className="mt-1 text-xs font-semibold text-muted-foreground">{label}</p></div>
}
...
function RequestDetails({ request }: { request: AutoCareServiceRequest | null }) {
    const { locale, t } = useTranslation()
    const [confirm, { isLoading: isConfirming, error: confirmError }] = useConfirmOwnerAutoCareServiceRequestMutation()
    ...
    const quote = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setQuoteValidationError(null)
        if (!Number.isFinite(total) || total <= 0) {
            setQuoteValidationError(t('autocare.ownerRequestsQuoteInvalidAmount'))
            return
        }
        const lineItems = [
            { kind: 'part' as const, title: t('autocare.ownerRequestsPartsPlaceholder'), quantity: 1, unitPriceMinor: Math.round(parts * 100) },
            { kind: 'labour' as const, title: t('autocare.ownerRequestsLabourPlaceholder'), quantity: 1, unitPriceMinor: Math.round(labour * 100) },
    ...
    const submitReschedule = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            await requestReschedule({ requestId: request.id, proposedAt: new Date(rescheduleAt).toISOString() }).unwrap()
        } catch {
            return
        }
        setRescheduleAt('')
    ...
    const handleNoShow = async () => {
        try {
            await markNoShow({ requestId: request.id }).unwrap()
        } catch {
            return
        }
    }
    const handleComplete = async () => {
        try {
            await complete({ requestId: request.id }).unwrap()
        } catch {
            return
        }
    }
    ...
    const confirmErrorMessage = confirmError ? getApiErrorMessage(confirmError, t('common.failedToLoad')) : null
    const preferredAt = request?.preferredAt
        ? formatDateTimeInTimeZone(request.preferredAt, locale, request.timezone || 'UTC')
        : t('autocare.ownerRequestsFlexibleTime')

    return <div className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3 border-b border-border pb-5"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">{t('autocare.ownerRequestsDetailEyebrow')}</p><h2 className="mt-2 text-xl font-black text-foreground">{request.serviceLabels[locale] ?? request.serviceLabels.en ?? request.serviceSlug}</h2><p className="mt-1 text-sm font-semibold text-muted-foreground">{request.providerName} · {request.address}</p></div><div className="flex items-center gap-2"><StatusBadge status={request.status} /><ContactClientLink request={request} /></div></div><div className="grid gap-4 border-b border-border py-5 sm:grid-cols-2"><Info label={t('autocare.ownerRequestsClient')} value={String(request.contactSnapshot?.name ?? t('common.notProvided'))} /><Info label={t('autocare.ownerRequestsVehicle')} value={vehicleText(request.vehicleSnapshot) ?? t('common.notProvided')} /><Info label={t('autocare.ownerRequestsPreferredTime')} value={preferredAt} /><Info label={t('autocare.ownerRequestsNote')} value={request.note ?? t('common.notProvided')} /></div>{request.booking ? <BookingSnapshot booking={request.booking} /> : null}<div className="grid gap-5 pt-5"><QuoteEditor locale={locale} request={request} amount={amount} partsAmount={partsAmount} labourAmount={labourAmount} note={note} priceLocked={priceLocked} isQuoting={isQuoting} quoteErrorMessage={quoteErrorMessage} quoteValidationError={quoteValidationError} onSubmit={quote} onAmount={setAmount} onPartsAmount={setPartsAmount} onLabourAmount={setLabourAmount} onNote={setNote} onPriceLocked={setPriceLocked} t={t} /></div>{request.status === 'accepted' && !request.reschedule && <form className="mt-5 rounded-[var(--radius-card)] border border-border bg-background p-4" onSubmit={(event) => void submitReschedule(event)}><p className="text-sm font-black text-foreground">{t('autocare.ownerRequestsRescheduleTitle')}</p><label className="mt-3 block text-xs font-semibold text-muted-foreground">{t('autocare.ownerRequestsRescheduleDate')}<input required type="datetime-local" value={rescheduleAt} onChange={(event) => setRescheduleAt(event.target.value)} className="mt-1 h-10 w-full rounded-[var(--radius-control)] border border-border bg-card px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /></label><button disabled={isRescheduling} className="mt-3 h-10 rounded-[var(--radius-control)] bg-primary px-4 text-xs font-black text-primary-foreground">{isRescheduling ? t('common.saving') : t('autocare.ownerRequestsRescheduleSend')}</button>{rescheduleError ? <p role="alert" className="mt-2 text-xs font-semibold text-destructive">{getApiErrorMessage(rescheduleError, t('common.failedToLoad'))}</p> : null}</form>}{canMarkNoShow ? <button type="button" disabled={isMarkingNoShow} onClick={() => void handleNoShow()} className="mt-5 inline-flex h-10 items-center gap-2 rounded-[var(--radius-control)] border border-destructive/40 px-4 text-xs font-black text-destructive hover:bg-destructive/5">{isMarkingNoShow ? t('common.saving') : t('autocare.ownerRequestsNoShow')}</button> : null}{noShowError ? <p role="alert" className="mt-2 text-xs font-semibold text-destructive">{getApiErrorMessage(noShowError, t('common.failedToLoad'))}</p> : null}{canComplete ? <button type="button" disabled={isCompleting} onClick={() => void handleComplete()} className="mt-5 inline-flex h-10 items-center gap-2 rounded-[var(--radius-control)] border border-primary px-4 text-xs font-black text-primary hover:bg-primary/5"><CheckCircle2 className="size-4" />{isCompleting ? t('common.saving') : t('autocare.ownerRequestsComplete')}</button> : null}{completionError ? <p role="alert" className="mt-2 text-xs font-semibold text-destructive">{getApiErrorMessage(completionError, t('common.failedToLoad'))}</p> : null}{['open', 'awaiting_reply', 'estimate_shared'].includes(request.status) && <button type="button" disabled={isConfirming} onClick={() => void confirm(request.id)} className="mt-5 inline-flex h-10 items-center gap-2 rounded-[var(--radius-control)] border border-primary px-4 text-xs font-black text-primary hover:bg-primary/5"><CheckCircle2 className="size-4" />{isConfirming ? t('common.saving') : t('autocare.ownerRequestsConfirm')}</button>}{confirmErrorMessage ? <p role="alert" className="mt-2 text-xs font-semibold text-destructive">{confirmErrorMessage}</p> : null}</div>
}

function BookingSnapshot({ booking }: { booking: NonNullable<AutoCareServiceRequest['booking']> }) {
    const { locale, t } = useTranslation()
    const date = formatDateTimeInTimeZone(booking.scheduledAt, locale, booking.timezone)
    return <div className="mt-5 flex items-start gap-2 rounded-[var(--radius-card)] border border-status-success-border bg-status-success-surface p-4 text-sm text-status-success-foreground"><CalendarCheck className="mt-0.5 size-4 shrink-0" /><div><p className="font-black">{t('autocare.ownerRequestsConfirmedBooking')}</p><p className="mt-1 text-xs font-semibold">{date} · {formatCurrency(booking.amountMinor / 100, booking.currencyCode, locale)}</p></div></div>
}

type QuoteEditorProps = { locale: string; request: AutoCareServiceRequest; amount: string; partsAmount: string; labourAmount: string; note: string; priceLocked: boolean; isQuoting: boolean; quoteErrorMessage: string | null; quoteValidationError: string | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onAmount: (value: string) => void; onPartsAmount: (value: string) => void; onLabourAmount: (value: string) => void; onNote: (value: string) => void; onPriceLocked: (value: boolean) => void; t: (key: TranslationKey) => string }
function QuoteEditor({ locale, request, amount, partsAmount, labourAmount, note, priceLocked, isQuoting, quoteErrorMessage, quoteValidationError, onSubmit, onAmount, onPartsAmount, onLabourAmount, onNote, onPriceLocked, t }: QuoteEditorProps) {
    const quoteExpired = request.quote?.status === 'expired'
    return <div><div className="flex items-center gap-2 text-sm font-black text-foreground"><Wrench className="size-4 text-primary" />{t('autocare.ownerRequestsQuoteTitle')}</div>{request.quote && !quoteExpired ? <div className="mt-3 rounded-[var(--radius-card)] border border-border bg-background p-4"><p className="text-2xl font-black text-foreground">{formatCurrency(request.quote.amountMinor / 100, request.quote.currencyCode, locale)}</p>{request.quote.lineItems?.map((item) => <p key={`${item.kind}-${item.title}`} className="mt-1 text-xs text-muted-foreground">{item.title}: {formatCurrency(item.totalMinor / 100, request.quote?.currencyCode ?? 'RUB', locale)}</p>)}<p className="mt-1 text-sm text-muted-foreground">{request.quote.note || t('autocare.ownerRequestsQuoteSent')}</p></div> : <>{quoteExpired ? <p className="mt-3 rounded-[var(--radius-card)] border border-status-warning-border bg-status-warning-surface px-3 py-2 text-xs font-semibold text-status-warning-foreground">{t('autocare.clientServiceRequestsQuoteExpired')}</p> : null}<form className="mt-3 space-y-3" onSubmit={onSubmit}><div className="grid gap-3 sm:grid-cols-3"><input required inputMode="decimal" value={amount} onChange={(event) => onAmount(event.target.value)} placeholder={t('autocare.ownerRequestsAmountPlaceholder')} aria-label={t('autocare.ownerRequestsAmountPlaceholder')} className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><input inputMode="decimal" value={partsAmount} onChange={(event) => onPartsAmount(event.target.value)} placeholder={t('autocare.ownerRequestsPartsPlaceholder')} aria-label={t('autocare.ownerRequestsPartsPlaceholder')} className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><input inputMode="decimal" value={labourAmount} onChange={(event) => onLabourAmount(event.target.value)} placeholder={t('autocare.ownerRequestsLabourPlaceholder')} aria-label={t('autocare.ownerRequestsLabourPlaceholder')} className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /></div><label className="flex items-center gap-2 text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={priceLocked} onChange={(event) => onPriceLocked(event.target.checked)} />{t('autocare.ownerRequestsPriceLock')}</label><textarea value={note} onChange={(event) => onNote(event.target.value)} placeholder={t('autocare.ownerRequestsQuoteNotePlaceholder')} aria-label={t('autocare.ownerRequestsQuoteNotePlaceholder')} className="min-h-20 w-full rounded-[var(--radius-control)] border border-border bg-background p-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><button disabled={isQuoting} className="h-10 rounded-[var(--radius-control)] bg-primary px-4 text-xs font-black text-primary-foreground">{isQuoting ? t('common.saving') : t('autocare.ownerRequestsSendQuote')}</button>{quoteValidationError ? <p role="alert" className="text-xs font-semibold text-status-warning-foreground">{quoteValidationError}</p> : null}{quoteErrorMessage && <p role="alert" className="text-xs font-semibold text-destructive">{quoteErrorMessage}</p>}</form></>}</div>
}

function ContactClientLink({ request }: { request: AutoCareServiceRequest }) {
    const { t } = useTranslation()
    const phone = request.contactSnapshot?.phone
    const label = t('autocare.ownerRequestsContactCustomer')
    return typeof phone === 'string' && phone.trim() ? <a href={`tel:${phone}`} className="inline-flex h-9 items-center gap-2 rounded-[var(--radius-control)] border border-primary/30 px-3 text-xs font-black text-primary transition hover:bg-primary/10"><Phone className="size-3.5" />{label}</a> : <span className="inline-flex h-9 items-center rounded-[var(--radius-control)] border border-border px-3 text-xs font-black text-muted-foreground">{t('autocare.ownerRequestsContactUnavailable')}</span>
}
