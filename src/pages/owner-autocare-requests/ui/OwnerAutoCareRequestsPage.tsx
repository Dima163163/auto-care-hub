...
function QuoteEditor({ request, amount, partsAmount, labourAmount, note, priceLocked, isQuoting, quoteErrorMessage, quoteValidationError, onSubmit, onAmount, onPartsAmount, onLabourAmount, onNote, onPriceLocked, t }: QuoteEditorProps) {
    const quoteExpired = request.quote?.status === 'expired'
    return <div><div className="flex items-center gap-2 text-sm font-black text-foreground"><Wrench className="size-4 text-primary" />{t('autocare.ownerRequestsQuoteTitle')}</div>{request.quote && !quoteExpired ? <div className="mt-3 rounded-[var(--radius-card)] border border-border bg-background p-4"><p className="text-2xl font-black text-foreground">{formatMoney(request.quote.amountMinor, request.quote.currencyCode)}</p>{request.quote.lineItems?.map((item) => <p key={`${item.kind}-${item.title}`} className="mt-1 text-xs text-muted-foreground">{item.title}: {formatMoney(item.totalMinor, request.quote?.currencyCode ?? 'RUB')}</p>)}<p className="mt-1 text-sm text-muted-foreground">{request.quote.note || t('autocare.ownerRequestsQuoteSent')}</p></div> : <>{quoteExpired ? <p className="mt-3 rounded-[var(--radius-card)] border border-status-warning-border bg-status-warning-surface px-3 py-2 text-xs font-semibold text-status-warning-foreground">{t('autocare.clientServiceRequestsQuoteExpired')}</p> : null}<form className="mt-3 space-y-3" onSubmit={onSubmit}><div className="grid gap-3 sm:grid-cols-3"><input required inputMode="decimal" value={amount} onChange={(event) => onAmount(event.target.value)} placeholder={t('autocare.ownerRequestsAmountPlaceholder')} aria-label={t('autocare.ownerRequestsAmountPlaceholder')} className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><input inputMode="decimal" value={partsAmount} onChange={(event) => onPartsAmount(event.target.value)} placeholder="Запчасти" aria-label="Запчасти" className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><input inputMode="decimal" value={labourAmount} onChange={(event) => onLabourAmount(event.target.value)} placeholder="Работа" aria-label="Работа" className="h-10 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /></div><label className="flex items-center gap-2 text-xs font-semibold text-muted-foreground"><input type="checkbox" checked={priceLocked} onChange={(event) => onPriceLocked(event.target.checked)} />Зафиксировать цену для этой заявки</label><textarea value={note} onChange={(event) => onNote(event.target.value)} placeholder={t('autocare.ownerRequestsQuoteNotePlaceholder')} aria-label={t('autocare.ownerRequestsQuoteNotePlaceholder')} className="min-h-20 w-full rounded-[var(--radius-control)] border border-border bg-background p-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><button disabled={isQuoting} className="h-10 rounded-[var(--radius-control)] bg-primary px-4 text-xs font-black text-primary-foreground">{isQuoting ? t('common.saving') : t('autocare.ownerRequestsSendQuote')}</button>{quoteValidationError ? <p role="alert" className="text-xs font-semibold text-status-warning-foreground">{quoteValidationError}</p> : null}{quoteErrorMessage && <p role="alert" className="text-xs font-semibold text-destructive">{quoteErrorMessage}</p>}</form></>}</div>
}
