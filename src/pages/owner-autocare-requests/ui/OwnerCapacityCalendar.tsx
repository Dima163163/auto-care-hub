...
export function OwnerCapacityCalendar({ requests, locale }: Props) {
    ...
    const capacityByLocation = new Map(branches.map(({ location }) => [location.id, location.appointmentCapacity ?? 1]))
    ...
    const branchesWithOccupancy = branches.map(({ provider, location }) => ({
        provider,
        location,
        requests: occupancy.get(location.id) ?? [],
        capacity: capacityByLocation.get(location.id) ?? 1,
    }))
    const ru = locale === 'ru'
    ...
    return <section className="mb-6 grid gap-4 rounded-[var(--radius-panel)] border border-border bg-card p-4 shadow-sm lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <div className="rounded-[var(--radius-card)] bg-secondary/50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-black text-foreground">{new Intl.DateTimeFormat(ru ? 'ru-RU' : 'en-US', { dateStyle: 'full' }).format(selectedDay)}</p><p className="mt-1 text-xs text-muted-foreground">{ru ? `${selected.length} подтверждённых записей` : `${selected.length} confirmed appointments`}</p></div><UsersRound className="size-5 text-primary" /></div>{providers.isLoading ? <div className="mt-5 h-20 animate-pulse rounded-[var(--radius-card)] bg-card" /> : branchesWithOccupancy.length === 0 ? <p className="mt-5 rounded-[var(--radius-card)] bg-card p-4 text-sm text-muted-foreground">{ru ? 'Нет доступных точек сервиса.' : 'No service locations are available.'}</p> : <div className="mt-5 space-y-3">{branchesWithOccupancy.map(({ provider, location, requests: branchRequests, capacity }) => <div key={location.id} className="rounded-[var(--radius-card)] bg-card p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-black text-foreground">{provider.name}</p><p className="mt-1 text-xs text-muted-foreground">{location.address}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-black ${branchRequests.length >= capacity ? 'bg-status-warning-surface text-status-warning-foreground' : 'bg-status-success-surface text-status-success-foreground'}`}>{branchRequests.length} / {capacity}</span></div>{branchRequests.length === 0 ? <p className="mt-3 text-xs text-muted-foreground">{ru ? 'Подтверждённых записей нет.' : 'No confirmed appointments.'}</p> : <div className="mt-3 grid gap-2 sm:grid-cols-2">{branchRequests.map((request) => <button key={request.id} type="button" className="rounded-[var(--radius-control)] border border-border px-3 py-2 text-left text-xs font-semibold text-foreground hover:border-primary">{new Intl.DateTimeFormat(ru ? 'ru-RU' : 'en-US', { timeStyle: 'short' }).format(new Date(request.preferredAt!))} · {request.serviceLabels.ru ?? request.serviceSlug}</button>)}</div>}<BranchResourceSummary providerId={provider.id} locationId={location.id} selectedDay={selectedDay} locale={locale} /></div>)}</div>}</div>
    </section>
}
