...
import { VirtualProviderList } from './VirtualProviderList'
...
export function AutoCareResultsPage() {
    ...
    const activeFocusedProviderId = providers.some((provider) => provider.id === focusedProviderId)
        ? focusedProviderId
        : null
    const updateFilters = (patch: Partial<AutoCareResultFilters>) => setSearchParams(
        (current) => writeAutoCareResultFilters(current, patch),
        { replace: true },
    )
    ...
    return (
        <main className="h-full min-h-0 bg-background lg:overflow-hidden">
            <div className="mx-auto flex h-full min-h-0 max-w-[var(--layout-operational-max)] flex-col px-[var(--layout-gutter)] py-6 sm:py-10">
                ...
                <div className="mt-6 grid gap-6 lg:min-h-0 lg:flex-1 lg:grid-cols-[minmax(0,1.04fr)_minmax(360px,0.76fr)]">
