...
import { formatCurrency, formatDistanceKm, parseDistanceKm } from '@/shared/lib/locale-format'
...
function markerMarkup(provider: ProviderPreview, serviceId: string, focused: boolean, priceLabel: string) {
    const tone = provider.trustBadge === 'trusted' || provider.verified ? 'success' : 'primary'
    ...
    return (
        <aside className="relative h-full min-h-[420px] overflow-hidden rounded-[var(--radius-panel)] border border-border bg-muted shadow-sm lg:min-h-0 lg:sticky lg:top-6" aria-label={t('autocare.mapPreviewLabel')}>
            ...
            <div className="absolute right-4 top-4 z-[500] flex flex-col overflow-hidden rounded-[var(--radius-control)] border border-primary-foreground/15 bg-hero-overlay/90 text-primary-foreground shadow-lg shadow-black/20 backdrop-blur">
                <button type="button" onClick={() => mapRef.current?.zoomIn()} className="inline-flex size-10 items-center justify-center border-b border-primary-foreground/15 hover:bg-primary-foreground/10" aria-label={t('cabinet.publicList.mapZoomIn')}><Plus className="size-4" /></button>
                <button type="button" onClick={() => mapRef.current?.zoomOut()} className="inline-flex size-10 items-center justify-center border-b border-primary-foreground/15 hover:bg-primary-foreground/10" aria-label={t('cabinet.publicList.mapZoomOut')}><Minus className="size-4" /></button>
