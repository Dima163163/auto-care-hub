...
export function AutoCareMapPreview({ providers, selectedProviders, focusedProviderId, onFocusProvider, onRemove }: AutoCareMapPreviewProps) {
    ...
    return <aside className="relative min-h-[620px] overflow-hidden rounded-[var(--radius-panel)] border border-border bg-muted shadow-sm lg:sticky lg:top-6" aria-label={t('autocare.mapPreviewLabel')}><div className="autocarehub-results-leaflet absolute inset-0"><div ref={mapContainerRef} className="h-full min-h-[620px] w-full" /></div><div className="absolute left-4 top-4 z-[500] flex items-center gap-2 rounded-[var(--radius-control)] border border-border bg-card/95 px-3 py-2 text-xs font-black text-foreground shadow-sm backdrop-blur"><MapIcon className="size-4 text-primary" />{t('autocare.mapPreviewLabel')}</div><div className="absolute right-4 top-4 z-[500] flex flex-col overflow-hidden rounded-[var(--radius-control)] border border-border bg-card/95 shadow-sm backdrop-blur"><button type="button" onClick={() => mapRef.current?.zoomIn()} className="inline-flex size-10 items-center justify-center border-b border-border hover:bg-muted" aria-label="Zoom in"><Plus className="size-4" /></button><button type="button" onClick={() => mapRef.current?.zoomOut()} className="inline-flex size-10 items-center justify-center border-b border-border hover:bg-muted" aria-label="Zoom out"><Minus className="size-4" /></button><button type="button" onClick={locate} className="inline-flex size-10 items-center justify-center hover:bg-muted" aria-label={t('cabinet.publicList.mapCurrentLocation')}><LocateFixed className="size-4" /></button></div>{tileError && <p className="absolute inset-x-4 top-20 z-[500] rounded-[var(--radius-control)] border border-status-warning-border bg-status-warning-surface px-3 py-2 text-xs font-semibold text-status-warning-foreground">{t('cabinet.publicList.mapTileError')}</p>}{locationStatus !== 'idle' && <p className="absolute right-4 top-40 z-[500] max-w-44 rounded-[var(--radius-control)] border border-border bg-card/95 p-2 text-xs font-semibold text-muted-foreground shadow-sm">{locationStatus === 'loading' ? t('cabinet.publicList.mapLocationLoading') : locationStatus === 'success' ? t('cabinet.publicList.mapLocationFound') : t('cabinet.publicList.mapLocationError')}</p>}{focusedProvider && <div className="absolute inset-x-4 bottom-4 z-[500] overflow-hidden rounded-[var(--radius-panel)] border border-border bg-card/95 shadow-xl backdrop-blur"><div className="flex items-start justify-between gap-3 border-b border-border px-4 py-3"><div><p className="text-xs font-bold text-primary">{t('autocare.trustedBadge')}</p><h2 className="mt-1 text-base font-black text-foreground">{focusedProvider.name}</h2></div><button type="button" onClick={() => onFocusProvider(null)} className="rounded-md p-1 text-muted-foreground hover:bg-muted" aria-label={t('common.close')}><X className="size-4" /></button></div><div className="flex items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-black text-foreground">{formatPrice(focusedProvider)}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">★ {focusedProvider.rating} · {focusedProvider.distance}</p></div><div className="flex gap-2"><button type="button" onClick={() => onRemove(focusedProvider.id)} className="inline-flex h-9 items-center rounded-[var(--radius-control)] border border-border px-3 text-xs font-bold text-foreground hover:border-primary">{selectedProviders.some((provider) => provider.id === focusedProvider.id) ? t('autocare.clearCompare') : t('autocare.compareAction')}</button><Link to={routePaths.serviceProviderDetails(focusedProvider.id)} className="inline-flex h-9 items-center rounded-[var(--radius-control)] bg-primary px-3 text-xs font-bold text-primary-foreground">{t('autocare.detailsAction')}</Link></div></div></div>}</aside>
}
