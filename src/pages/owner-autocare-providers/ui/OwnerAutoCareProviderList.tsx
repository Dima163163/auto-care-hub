    ...
    return <div className="grid gap-3 md:grid-cols-2">
        {providers.map((provider) => {
            ...
            return <article key={provider.id} data-testid="owner-provider-card" className="rounded-xl border bg-card p-5 shadow-sm transition hover:border-primary hover:shadow-md">
                ...
                <div className="mt-4 grid min-w-0 gap-3 border-t border-border pt-3 text-xs font-bold lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                        <span className="inline-flex min-w-0 items-center gap-1.5 text-muted-foreground">{chatEnabled ? <MessageCircle className="size-3.5 shrink-0 text-primary" /> : <Phone className="size-3.5 shrink-0 text-primary" />}<span className="truncate">{chatEnabled ? (locale === 'ru' ? 'Чаты включены' : 'Chat enabled') : <>{locale === 'ru' ? 'Связь по телефону' : 'Phone contact'}<span className="sr-only">{modeLabel}</span></>}</span></span>
