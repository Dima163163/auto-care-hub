...
import { automotiveAmenities, getAutomotiveAmenityLabel, type AutoCareApiProvider, type AutomotiveAmenity } from '@/entities/automotive-service'
...
export function OwnerAutoCareProviderList({ providers }: OwnerAutoCareProviderListProps) {
    ...
    return <div className="grid gap-3 md:grid-cols-2">
        {providers.map((provider) => {
            ...
            return <article key={provider.id} className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold">{provider.name}</h2><p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground"><MapPin className="size-3.5" />{provider.location.address}</p></div><span className="rounded-full bg-status-warning-surface px-2.5 py-1 text-xs font-bold text-status-warning-foreground">{provider.status === 'active' ? t('autocare.ownerProviderPublished') : t('autocare.ownerProviderDraft')}</span></div><p className="mt-4 line-clamp-2 min-h-10 text-sm leading-5 text-muted-foreground">{provider.description || t('common.notProvided')}</p><p className="mt-4 text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">{t('autocare.ownerProviderAmenitiesCount', { count: amenities.length })}</p><div className="mt-2 flex flex-wrap gap-1.5">{amenities.map((amenity) => <span key={amenity.id} className="rounded-full bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">{getAutomotiveAmenityLabel(amenity, locale)}</span>)}</div></article>
        })}
    </div>
}
