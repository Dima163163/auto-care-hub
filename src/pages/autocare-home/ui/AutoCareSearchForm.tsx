...
export function AutoCareSearchForm({ marketId, markets, onMarketChange }: AutoCareSearchFormProps) {
    ...
    const marketOptions = markets.length > 0
        ? markets.map((market) => ({ value: market.cityCode, label: `${market.cityName}, ${market.countryName}` }))
        : MARKET_REGISTRY.map((market) => ({ value: market.id === 'ru-moscow' ? 'moscow' : market.cityCode.toLowerCase(), label: `${market.cityName}, ${market.countryName}` }))
