                ...
                {provider.offerings.map((offering) => {
                    ...
                    return (
                        <article key={offering.serviceId} className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_7rem_9rem] sm:items-center">
                            ...
                            <div className="flex items-center justify-between gap-3 sm:block"><strong className="text-sm font-black text-foreground">{priceLabel}</strong>{offering.warrantyText && <span className="mt-1 block text-xs font-medium text-muted-foreground">{offering.warrantyText}</span>}</div>
