...
import { getPublicTrustEvidence } from '../lib/providerTrust'

export function AutoCareProviderPage() {
    const { id = '' } = useParams()
    const [searchParams, setSearchParams] = useSearchParams()
    const { t } = useTranslation()
    const providerQuery = useGetAutoCareProviderProfileQuery(id, { skip: !id })
    const reviewsQuery = useGetAutoCareProviderReviewsQuery({ providerId: id, limit: 50 }, { skip: !id })
    const trustQuery = useGetAutoCareProviderTrustQuery(id, { skip: !id })
    const { data, isLoading, isFetching, refetch } = providerQuery
    const { data: reviewSummary } = reviewsQuery
    const { data: trust } = trustQuery
    const provider = data ? mapAutoCareProviderProfile(data, reviewSummary) : undefined
    const providerErrorState = getApiErrorState(providerQuery.error)
    const providerState = resolveQueryViewState({
        isLoading,
        isFetching,
        isError: providerQuery.isError,
        hasData: Boolean(data),
        hasResults: Boolean(provider),
        isOffline: providerErrorState === 'offline',
        isPermissionDenied: providerErrorState === 'permission-denied',
    ...
    const reviewsState = resolveQueryViewState({
        ...
        hasData: Boolean(reviewSummary),
        hasResults: (reviewSummary?.reviews.length ?? 0) > 0,
        isOffline: reviewsErrorState === 'offline',
        isPermissionDenied: reviewsErrorState === 'permission-denied',
        isSuspended: reviewsErrorState === 'suspended',
        isStale: reviewsErrorState === 'stale',
        isSessionExpired: reviewsErrorState === 'session-expired',
    })
    const [selectedServiceId, setSelectedServiceId] = useState('')
    const requestedServiceId = searchParams.get('service') ?? ''
    const selectedOffering = useMemo(() => provider?.offerings.find((item) => item.serviceId === selectedServiceId)
        ?? provider?.offerings.find((item) => item.serviceId === requestedServiceId)
        ?? provider?.offerings[0], [provider, requestedServiceId, selectedServiceId])
    ...
    if (!provider) {
        ...
        return <main className="mx-auto max-w-[var(--layout-public-wide-max)] px-[var(--layout-public-gutter)] py-20"><QueryStateCard state={unavailableState} error={providerQuery.error} emptyTitle={t('autocare.providerNotFound')} emptyDescription={t('autocare.providerNoOffersDescription')} suspendedTitle={t('autocare.providerSuspendedTitle')} suspendedDescription={t('autocare.providerSuspendedDescription')} onRetry={() => { void refetch() }} /></main>
    }
    if (provider.status === 'suspended') return <main className="mx-auto max-w-[var(--layout-public-wide-max)] px-[var(--layout-public-gutter)] py-20"><StateCard variant="suspended" title={t('autocare.providerSuspendedTitle')} description={t('autocare.providerSuspendedDescription')} /></main>
    if (provider.status === 'draft') return <main className="mx-auto max-w-[var(--layout-public-wide-max)] px-[var(--layout-public-gutter)] py-20"><StateCard variant="empty" title={t('autocare.providerNotFound')} description={t('autocare.providerNoOffersDescription')} /></main>
    if (!selectedOffering) return <main className="mx-auto max-w-[var(--layout-public-wide-max)] px-[var(--layout-public-gutter)] py-20"><StateCard variant="empty" title={t('autocare.providerNoOffersTitle')} description={t('autocare.providerNoOffersDescription')} /></main>

    return <><ProviderHero provider={provider} selectedServiceId={selectedOffering.serviceId} /><ProviderSectionNavigation /><main data-testid="provider-page-main" className="mx-auto grid max-w-[var(--layout-public-wide-max)] gap-6 px-[var(--layout-public-gutter)] py-7 sm:py-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.42fr)]"><div className="grid content-start gap-6">{providerState === 'stale-error' && <QueryStateCard state="stale-error" error={providerQuery.error} onRetry={() => { void refetch() }} />}{providerState === 'partial' && <QueryStateCard state="partial" onRetry={() => { void refetch() }} />}<QueryRefreshStatus isRefreshing={isFetching && !isLoading} label={t('common.refreshing')} /><ProviderOfferings provider={provider} selectedServiceId={selectedOffering.serviceId} onSelect={selectService} /><ProviderAbout provider={provider} trust={trust} trustError={trustQuery.error} onRetryTrust={() => { void trustQuery.refetch() }} /><ProviderLocation provider={provider} /><ProviderReviews provider={provider} state={reviewsState} error={reviewsQuery.error} onRetry={() => { void reviewsQuery.refetch() }} /></div><ProviderRequestPanel provider={provider} offering={selectedOffering} /></main></>
}
...
function ProviderLocation({ provider }: { provider: NonNullable<ReturnType<typeof mapAutoCareProviderProfile>> }) {
    const { t, locale } = useTranslation()
    const schedule = Object.entries(provider.weeklySchedule ?? {})
    return <section id="location" className="overflow-hidden rounded-[var(--radius-panel)] border border-border bg-card shadow-sm"><div className="grid gap-4 p-5 sm:grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] sm:p-6"><div><h2 className="text-xl font-black tracking-tight text-foreground">{t('autocare.providerFindUs')}</h2><div className="mt-4 grid gap-3 text-sm font-semibold text-muted-foreground"><p className="flex items-start gap-2"><MapPinned className="mt-0.5 size-4 shrink-0 text-primary" />{provider.address}</p>{provider.metroStation && <p className="flex items-start gap-2"><MapPinned className="mt-0.5 size-4 shrink-0 text-primary" />{provider.metroStation}</p>}<p className="flex items-start gap-2"><Clock3 className="mt-0.5 size-4 shrink-0 text-primary" />{provider.hours}</p>{provider.phones.length > 0 ? provider.phones.map((phone) => <p key={phone} className="flex items-start gap-2"><Phone className="mt-0.5 size-4 shrink-0 text-primary" />{phone}</p>) : provider.phone && <p className="flex items-start gap-2"><Phone className="mt-0.5 size-4 shrink-0 text-primary" />{provider.phone}</p>}{provider.email && <p className="flex items-start gap-2"><Mail className="mt-0.5 size-4 shrink-0 text-primary" />{provider.email}</p>}{provider.websiteUrl && <a href={provider.websiteUrl} target="_blank" rel="noreferrer" className="flex items-start gap-2 text-primary hover:underline"><Globe2 className="mt-0.5 size-4 shrink-0" />{provider.websiteUrl}</a>}</div>{schedule.length > 0 && <details className="mt-4 rounded-[var(--radius-control)] border border-border px-3 py-2 text-xs"><summary className="cursor-pointer font-black text-foreground">{t('autocare.providerLocation')}</summary><div className="mt-3 grid gap-1.5 text-muted-foreground">{schedule.map(([day, hours]) => <div key={day} className="flex items-center justify-between gap-3"><span>{getScheduleDayLabel(day, locale)}</span><span className={hours.closed ? 'text-status-warning-foreground' : 'text-foreground'}>{hours.closed ? '—' : `${hours.open}–${hours.close}`}</span></div>)}</div></details>}<div className="mt-4 flex flex-wrap gap-2">{provider.supportsMobile && <span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">{provider.coverageRadiusKm ? t('autocare.providerMobileServiceWithRadius', { radius: provider.coverageRadiusKm.toLocaleString(locale) }) : t('autocare.providerMobileService')}</span>}{provider.supportsPickup && <span className="rounded-full bg-status-success-surface px-2.5 py-1 text-xs font-bold text-status-success-foreground">{t('autocare.providerPickupDelivery')}</span>}</div></div><ProviderLocationMap provider={provider} /></div></section>
}

function getScheduleDayLabel(day: string, locale: string) {
    const index = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].indexOf(day)
    if (index < 0) return day
    return new Intl.DateTimeFormat(locale, { weekday: 'long' }).format(new Date(Date.UTC(2026, 0, 4 + index)))
}
