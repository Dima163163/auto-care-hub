...
export function AutoCareProviderPage() {
    ...
    if (!provider) return <main className="mx-auto max-w-[var(--layout-public-max)] px-[var(--layout-gutter)] py-20"><QueryStateCard state={providerState === 'empty' ? 'error' : providerState} error={providerQuery.error} onRetry={refetch} /></main>
    ...
    return <><ProviderHero provider={provider} /><ProviderSectionNavigation /><main className="mx-auto grid max-w-[var(--layout-operational-max)] gap-6 px-[var(--layout-gutter)] py-7 sm:py-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.42fr)]"><div className="grid content-start gap-6">{providerState === 'stale-error' && <QueryStateCard state="stale-error" error={providerQuery.error} onRetry={refetch} />}<QueryRefreshStatus isRefreshing={isFetching && !isLoading} label={t('common.refreshing')} /><ProviderOfferings provider={provider} selectedServiceId={selectedOffering.serviceId} onSelect={setSelectedServiceId} /><ProviderAbout provider={provider} trust={trust} trustError={trustQuery.error} onRetryTrust={trustQuery.refetch} /><ProviderLocation provider={provider} /><ProviderReviews provider={provider} state={reviewsState} error={reviewsQuery.error} onRetry={reviewsQuery.refetch} /></div><ProviderRequestPanel provider={provider} offering={selectedOffering} /></main></>
}

function ProviderAbout({ provider, trust, trustError, onRetryTrust }: { provider: NonNullable<ReturnType<typeof mapAutoCareProviderProfile>>; trust?: AutoCareTrustResponse; trustError?: unknown; onRetryTrust: () => unknown }) {
