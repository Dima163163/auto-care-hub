...
export function ProviderReviews({ provider, state, error, onRetry }: { provider: ProviderProfile; state: QueryViewState; error?: unknown; onRetry: () => void }) {
    ...
    if (state === 'error' || state === 'offline' || state === 'permission-denied' || state === 'suspended') return <section id="reviews" className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm sm:p-6"><QueryStateCard state={state} error={error} onRetry={onRetry} /></section>
