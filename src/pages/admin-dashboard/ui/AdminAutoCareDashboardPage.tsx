...
export function AdminAutoCareDashboardPage() {
    ...
    return <main className="min-h-full bg-background px-[var(--layout-gutter)] py-7 lg:py-10"><section className="mx-auto max-w-6xl space-y-5"><AdminAutoCareDashboardHero locale={locale} pendingCount={providerStats.draft} />{isLoading && <div className="rounded-[var(--radius-panel)] border border-border bg-card p-8 text-sm font-semibold text-muted-foreground">{t('common.loading')}</div>}{error && <div className="rounded-[var(--radius-panel)] border border-destructive/30 bg-card p-6"><p className="font-semibold text-destructive">{getApiErrorMessage(error, t('common.failedToLoad'))}</p><RetryButton className="mt-4" onRetry={() => void Promise.all([providers.refetch(), users.refetch()])} label={t('common.retry')} /></div>}{!isLoading && !error && <><AdminAutoCareMetricGrid locale={locale} providers={providerStats} users={userStats} /><AdminAutoCareModerationQueue locale={locale} providers={providerData} /></>}</section></main>
}
