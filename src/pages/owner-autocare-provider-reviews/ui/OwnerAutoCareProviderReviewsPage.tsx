...
export function ReviewResolutionDialog({ providerId, review, copy }: { providerId: string; review: AutoCareApiReview; copy: ReviewResolutionCopy }) {
    const [isOpen, setIsOpen] = useState(false)
    ...
    const close = () => { setIsOpen(false); setPromoCode(null); setCopied(false); setActionError(null) }
    const submit = async () => {
        setActionError(null)
        try {
            ...
            setActionError(getApiErrorMessage(error, copy.promoError))
        }
    }
    const copyCode = async () => {
        ...
        try {
            ...
            setActionError(getApiErrorMessage(error, copy.copyError))
        }
    }
    if (!review.serviceRequestId) return null
    return <><button type="button" onClick={() => { setActionError(null); setIsOpen(true) }} className="inline-flex items-center gap-1.5 rounded-[var(--radius-control)] bg-primary px-3 py-2 text-xs font-black text-primary-foreground transition hover:bg-primary/90"><Gift className="size-3.5" />{copy.issueDiscount}</button><Dialog isOpen={isOpen} onOpenChange={(open) => open ? setIsOpen(true) : close()} className="max-w-md"><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2"><Tag className="size-5 text-primary" />{copy.issueDiscount}</DialogTitle><DialogDescription>{copy.promoNote}</DialogDescription></DialogHeader>{promoCode ? <div className="mt-5 rounded-[var(--radius-card)] border border-status-success-foreground/30 bg-status-success-surface p-4"><p className="text-sm font-black text-status-success-foreground">{copy.promoCreated}</p><div className="mt-3 flex items-center gap-2"><code className="flex-1 rounded-[var(--radius-control)] bg-background px-3 py-2 text-center text-lg font-black tracking-widest text-foreground">{promoCode}</code><button type="button" onClick={() => void copyCode()} aria-label={copy.copyCode} className="flex size-10 items-center justify-center rounded-[var(--radius-control)] border border-border bg-card text-primary hover:bg-primary/10">{copied ? <Check className="size-4" /> : <Copy className="size-4" />}</button></div><p className="mt-2 text-xs text-muted-foreground">{copy.copyCode}</p></div> : <div className="mt-5 grid gap-4"><label className="grid gap-1.5 text-sm font-bold text-foreground"><span>{copy.discountPercent}</span><input type="number" min="1" max="100" value={discountPercent} onChange={(event) => { setActionError(null); setDiscountPercent(event.target.value) }} aria-invalid={Boolean(actionError) || undefined} aria-describedby={actionError ? 'owner-review-promo-error' : undefined} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3" /></label><label className="grid gap-1.5 text-sm font-bold text-foreground"><span>{copy.expiresInDays}</span><input type="number" min="1" max="90" value={expiresInDays} onChange={(event) => { setActionError(null); setExpiresInDays(event.target.value) }} aria-invalid={Boolean(actionError) || undefined} aria-describedby={actionError ? 'owner-review-promo-error' : undefined} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3" /></label></div>}{actionError ? <p id="owner-review-promo-error" role="alert" className="mt-4 text-sm font-bold text-destructive">{actionError}</p> : null}<DialogFooter>{promoCode ? <button type="button" onClick={close} className="inline-flex h-10 items-center justify-center gap-2 rounded-[var(--radius-control)] bg-primary px-4 text-sm font-black text-primary-foreground"><X className="size-4" />{copy.close}</button> : <><button type="button" onClick={close} className="inline-flex h-10 items-center justify-center rounded-[var(--radius-control)] border border-border px-4 text-sm font-black text-foreground">{copy.cancel}</button><button type="button" disabled={issueState.isLoading} onClick={() => void submit()} className="inline-flex h-10 items-center justify-center gap-2 rounded-[var(--radius-control)] bg-primary px-4 text-sm font-black text-primary-foreground disabled:opacity-60"><Gift className="size-4" />{copy.issue}</button></>}</DialogFooter></DialogContent></Dialog></>
}
