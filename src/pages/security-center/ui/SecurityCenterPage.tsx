...
function maskIpAddress(value: string | null, reveal: boolean) {
    if (!value || reveal) return value
    if (value.includes(':')) return `${value.split(':').slice(0, 3).join(':')}:…`
    const parts = value.split('.')
    return parts.length === 4 ? `${parts.slice(0, 3).join('.')}.*` : '•••'
}

function maskUserAgent(value: string | null, reveal: boolean) {
    if (!value || reveal) return value
...
export function SecurityCenterPage() {
    ...
    const canView = currentUser?.role === 'admin' || currentUser?.role === 'super_admin'
    const canRevokeSessions = currentUser?.role === 'super_admin'
    const canViewSensitive = currentUser?.role === 'super_admin'
            ...
            <section className="mx-auto max-w-7xl">
                <PageHeader
                    ...
                    actions={(
                        <div className="flex flex-wrap gap-2">
                            {canViewSensitive && <Button type="button" variant="outline" className="min-h-11" onClick={() => void downloadReport()} loading={isExporting}>
                            ...
                            {canRevokeSessions && selectedEvent?.userId && <Button type="button" variant="outline" className="min-h-11" onClick={() => void revokeSelectedUserSessions()} loading={isRevokingSessions}>
                ...
                {hasError && !summary && !eventsPage ? (
                    ...
                    <>
                        ...
                        {canRevokeSessions && <SecurityMitigationsPanel />}
                        ...
                        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_1fr]">
                            <section className="rounded-lg border bg-card p-5">
                                ...
                                <div className="mt-4 space-y-2">
                                    ...
                                    {(summary?.topIps ?? []).map((item) => <div key={item.ipAddress} className="flex items-center justify-between gap-4 text-sm"><code className="truncate text-muted-foreground">{maskIpAddress(item.ipAddress, canViewSensitive)}</code><span className="font-semibold">{item.count}</span></div>)}
                                </div>
                            </section>
                            <section className="rounded-lg border bg-card p-5">
                        ...
                        <div className="mt-6 grid gap-6 xl:grid-cols-3">
                            ...
                            <section className="rounded-lg border bg-card p-5">
                                ...
                                <div className="mt-4 space-y-2">
                                    ...
                                    {(summary?.topUserAgents ?? []).map((item) => <div key={item.userAgent} className="flex items-center justify-between gap-4 text-sm"><code className="truncate text-muted-foreground">{maskUserAgent(item.userAgent, canViewSensitive)}</code><span className="font-semibold">{item.count}</span></div>)}
                                </div>
                            </section>
                            <section className="rounded-lg border bg-card p-5">
                        ...
                        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                            ...
                            <aside data-testid="security-center-detail-drawer" className={`rounded-lg border bg-card p-5 ${selectedEvent ? 'max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-[1100] max-md:max-h-[85dvh] max-md:overflow-y-auto max-md:rounded-b-none max-md:pb-[calc(1.25rem+var(--mobile-nav-height)+env(safe-area-inset-bottom))] max-md:shadow-2xl' : 'max-md:hidden'}`} aria-label={t('securityCenter.details')}>
                                ...
                                {selectedEvent ? <><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">{t('securityCenter.details')}</p><h2 className="mt-2 font-semibold">{t(typeKeys[selectedEvent.type])}</h2></div><button type="button" className="text-muted-foreground hover:text-foreground" onClick={() => { setSelectedId(null); setOperatorNote('') }} aria-label={t('common.close')}><XCircle className="size-5" /></button></div><dl className="mt-5 space-y-3 text-sm"><div><dt className="text-muted-foreground">{t('securityCenter.ipFilter')}</dt><dd className="mt-1 break-all font-mono">{maskIpAddress(selectedEvent.ipAddress, canViewSensitive) ?? t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.routeColumn')}</dt><dd className="mt-1 break-all font-mono">{selectedEvent.method ?? ''} {selectedEvent.route ?? t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.requestId')}</dt><dd className="mt-1 break-all font-mono">{selectedEvent.requestId ?? t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.actorRole')}</dt><dd className="mt-1">{selectedEvent.actorRole ? t(actorRoleKeys[selectedEvent.actorRole]) : t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.authOutcome')}</dt><dd className="mt-1">{t(authOutcomeKeys[selectedEvent.authOutcome])}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.rateLimitResult')}</dt><dd className="mt-1">{t(rateLimitResultKeys[selectedEvent.rateLimitResult])}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.proxyProvenance')}</dt><dd className="mt-1">{t(proxyProvenanceKeys[selectedEvent.proxyProvenance])}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.requestSize')}</dt><dd className="mt-1">{selectedEvent.requestSizeBytes === null ? t('securityCenter.notAvailable') : `${selectedEvent.requestSizeBytes} ${t('securityCenter.bytes')}`}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.reasonCode')}</dt><dd className="mt-1 break-all font-mono">{selectedEvent.reasonCode ?? t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.userAgent')}</dt><dd className="mt-1 break-words text-muted-foreground">{maskUserAgent(selectedEvent.userAgent, canViewSensitive) ?? t('securityCenter.notAvailable')}</dd></div><div><dt className="text-muted-foreground">{t('securityCenter.metadata')}</dt><dd className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-words rounded bg-muted p-2 font-mono text-xs">{canViewSensitive ? JSON.stringify(selectedEvent.metadata, null, 2) : '[redacted — super-admin access required]'}</dd></div></dl><div className="mt-5 border-t pt-4"><label className="text-sm font-medium" htmlFor="security-center-operator-note">{t('securityCenter.operatorNote')}</label><textarea id="security-center-operator-note" value={operatorNote} onChange={(event) => setOperatorNote(event.target.value)} maxLength={1_000} rows={3} placeholder={t('securityCenter.operatorNotePlaceholder')} className="mt-2 w-full resize-y rounded-md border bg-background px-3 py-2 text-sm" /><p className="mt-1 text-xs text-muted-foreground">{operatorNote.length}/1000</p><p className="mt-3 text-sm font-medium">{t('securityCenter.changeStatus')}</p><div className="mt-3 flex flex-wrap gap-2">{(['acknowledged', 'investigating', 'resolved'] as const).map((nextStatus) => <Button key={nextStatus} type="button" variant="outline" size="sm" disabled={selectedEvent.status === nextStatus || isUpdating} loading={isUpdating && pendingAction?.status === nextStatus} onClick={() => void changeStatus(nextStatus)}>{t(statusKeys[nextStatus])}</Button>)}</div></div></> : <div className="flex min-h-52 flex-col items-center justify-center text-center"><ShieldAlert className="size-9 text-muted-foreground" /><p className="mt-3 font-medium">{t('securityCenter.selectEvent')}</p><p className="mt-1 text-sm text-muted-foreground">{t('securityCenter.selectEventDescription')}</p></div>}
