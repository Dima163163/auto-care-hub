...
function RequestCard({ request, selected, onSelect }: { request: AutoCareServiceRequest; selected: boolean; onSelect: () => void }) {
    ...
    return <button type="button" onClick={onSelect} className={`min-w-0 rounded-[var(--radius-card)] border p-4 text-left transition ${selected ? 'border-primary bg-primary/5' : 'border-border bg-background hover:border-primary/50'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 break-words"><p className="text-sm font-black text-foreground">{serviceLabel}</p><p className="mt-1 text-xs font-semibold text-muted-foreground">{request.providerName}</p></div><Status status={request.status} /></div><div className="mt-4 flex flex-wrap items-center gap-3 text-xs font-semibold text-muted-foreground"><span>{request.preferredAt ? formatDateTime(request.preferredAt, locale, { dateStyle: 'medium', timeStyle: 'short', ...(request.timezone ? { timeZone: request.timezone } : {}) }) : t('autocare.clientServiceRequestsFlexible')}{request.preferredAt && request.timezone ? ` · ${t('autocare.clientServiceRequestsServiceTime')}: ${request.timezone}` : ''}</span>{request.priceFromMinor !== null && request.currencyCode ? <span className="font-black text-foreground">{t('autocare.clientServiceRequestsBookedPrice')}: {formatMoney(request.priceFromMinor, request.currencyCode, locale)}</span> : null}{request.quote ? <span className="font-black text-primary">{formatMoney(request.quote.amountMinor, request.quote.currencyCode, locale)}</span> : null}</div>{request.booking ? <BookingSnapshot booking={request.booking} /> : null}<p className="mt-3 text-xs font-bold text-primary">{t('autocare.clientServiceRequestsOpen')}</p></button>
}
...
function Conversation({ request, onClose }: { request: AutoCareServiceRequest; onClose: () => void }) {
    ...
    const handleRescheduleDecision = async (decision: 'accept' | 'reject') => {
        ...
        setQuoteActionError(null)
        try {
            if (!request.reschedule) return
            await decideReschedule({ requestId: request.id, rescheduleId: request.reschedule.id, decision }).unwrap()
        } catch (error) {
