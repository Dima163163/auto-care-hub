...
function BonusSummary({ accounts }: { accounts: AutoCareBonusAccount[] }) {
    ...
    return <div className="mt-4 grid gap-2 sm:grid-cols-2">{accounts.map((account) => <details key={account.id} className="group rounded-[var(--radius-card)] border border-primary/20 bg-primary/5 px-3 py-2"><summary className="cursor-pointer list-none"><p className="text-[10px] font-black uppercase tracking-wide text-primary">{isRu ? 'Бонусы сервиса' : 'Service bonuses'}</p><p className="mt-1 text-lg font-black text-foreground">{account.balancePoints.toLocaleString(locale)} <span className="text-xs font-bold text-muted-foreground">{isRu ? 'баллов' : 'points'}</span></p><p className="text-[11px] font-semibold text-muted-foreground">{isRu ? 'Начислено' : 'Earned'}: {account.earnedPoints.toLocaleString(locale)} · {isRu ? 'История' : 'History'}: {account.entries.length}</p></summary>{account.entries.length > 0 && <div className="mt-3 border-t border-primary/15 pt-2"><p className="text-[10px] font-black uppercase tracking-wide text-muted-foreground">{isRu ? 'Операции' : 'Transactions'}</p><ul className="mt-2 grid gap-1.5">{account.entries.slice(0, 8).map((entry) => <li key={entry.id} className="flex items-start justify-between gap-2 text-[11px]"><span className="min-w-0 text-muted-foreground">{entry.reason}</span><b className={entry.points >= 0 ? 'text-status-success-foreground' : 'text-status-danger-foreground'}>{entry.points > 0 ? '+' : ''}{entry.points}</b></li>)}</ul></div>}</details>)}</div>
}
