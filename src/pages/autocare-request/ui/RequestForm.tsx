...
function AppointmentPicker({ selectedDate, selectedTime, onDateChange, onTimeChange }: { selectedDate: string; selectedTime: string; onDateChange: (date: string) => void; onTimeChange: (time: string) => void }) {
    const { t } = useTranslation()
    return <section><div className="flex items-center gap-2"><CalendarDays className="size-5 text-primary" /><h2 className="text-xl font-black tracking-tight text-foreground">{t('autocare.requestDateLabel')} и время</h2></div><div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)]"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">{t('autocare.requestDateLabel')}</p><div className="mt-3 grid grid-cols-2 gap-2">{appointmentDates.map((date) => <button key={date} type="button" onClick={() => onDateChange(date)} className={selectedDate === date ? 'h-11 rounded-[var(--radius-control)] border border-primary bg-primary/10 text-xs font-black text-primary' : 'h-11 rounded-[var(--radius-control)] border border-border text-xs font-bold text-foreground transition hover:border-primary hover:text-primary'}>{date}</button>)}</div></div><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">{t('autocare.requestTimeLabel')}</p><div className="mt-3 grid grid-cols-3 gap-2">{appointmentTimes.map((time) => <button key={time} type="button" onClick={() => onTimeChange(time)} className={selectedTime === time ? 'h-10 rounded-[var(--radius-control)] border border-primary bg-primary text-xs font-black text-primary-foreground shadow-sm' : 'h-10 rounded-[var(--radius-control)] border border-border text-xs font-bold text-foreground transition hover:border-primary hover:text-primary'}>{time}</button>)}</div></div></div><p className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-muted-foreground"><Clock3 className="size-3.5 text-primary" />Выбрано: {selectedDate}, {selectedTime}</p></section>
}

function VehicleAndContacts() {
    const { t } = useTranslation()
    return <section className="border-t border-border pt-6"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black tracking-tight text-foreground">{t('autocare.requestContactTitle')}</h2><span className="inline-flex items-center gap-1.5 text-xs font-bold text-status-success-foreground"><Check className="size-3.5" />Данные защищены</span></div><div className="mt-4 grid gap-4 sm:grid-cols-3"><input required aria-label={t('autocare.requestNamePlaceholder')} placeholder={t('autocare.requestNamePlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /><input required aria-label={t('autocare.requestPhonePlaceholder')} placeholder={t('autocare.requestPhonePlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /><input type="email" required aria-label={t('autocare.requestEmailPlaceholder')} placeholder={t('autocare.requestEmailPlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /></div><div className="mt-4 flex items-center justify-between gap-4 rounded-[var(--radius-card)] bg-secondary p-4"><div><p className="text-sm font-black text-secondary-foreground">BMW X5, 2021</p><p className="mt-1 text-xs font-medium text-muted-foreground">Автомобиль помогает сервису точнее оценить работу.</p></div><button type="button" className="text-xs font-black text-primary">Изменить</button></div></section>
}
