...
const appointmentDates = [
    { label: 'Сегодня', date: '24 мая' },
    { label: 'Завтра', date: '25 мая' },
    { label: 'Пт', date: '26 мая' },
    { label: 'Сб', date: '27 мая' },
]
...
export function RequestForm({ onSubmit }: RequestFormProps) {
    ...
    return (
        <form onSubmit={onSubmit} className="grid gap-5 rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm sm:p-6">
            <AppointmentPicker locale={locale} selectedDate={selectedDate} selectedTime={selectedTime} onDateChange={setSelectedDate} onTimeChange={setSelectedTime} />
            <VehicleAndContacts />
            <RequestDetails />
...
function AppointmentPicker({ locale, selectedDate, selectedTime, onDateChange, onTimeChange }: AppointmentPickerProps) {
    const { t } = useTranslation()
    const [customDate, setCustomDate] = useState('')
    const days = appointmentDates.map(({ label }, index) => ({
        id: label,
            ...
            <div className="mt-4 grid gap-5 sm:grid-cols-[minmax(220px,0.75fr)_minmax(0,1fr)]">
                <div className="rounded-[var(--radius-card)] border border-border p-4">
                    ...
                    <div className="mt-3 grid grid-cols-4 gap-1.5">
                        {days.map(({ id, label, date }) => <button key={id} type="button" onClick={() => { setCustomDate(''); onDateChange(id) }} className={!customDate && selectedDate === id ? 'min-h-14 rounded-[var(--radius-control)] border border-primary bg-primary/10 px-1 text-[10px] font-black text-primary' : 'min-h-14 rounded-[var(--radius-control)] border border-border px-1 text-[10px] font-bold text-muted-foreground transition hover:border-primary hover:text-primary'}><span className="block">{label}</span><span className="mt-1 block text-[9px] font-medium">{date}</span></button>)}
                    </div>
...
function VehicleAndContacts() {
    ...
    return (
        <section className="grid gap-5 border-t border-border pt-5">
            ...
            <div><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black tracking-tight text-foreground">{t('autocare.requestContactTitle')}</h2><span className="inline-flex items-center gap-1.5 text-xs font-bold text-status-success-foreground"><Check className="size-3.5" />{t('autocare.requestDataSecure')}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><input required aria-label={t('autocare.requestNamePlaceholder')} placeholder={t('autocare.requestNamePlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /><input required aria-label={t('autocare.requestPhonePlaceholder')} placeholder={t('autocare.requestPhonePlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /><input type="email" required aria-label={t('autocare.requestEmailPlaceholder')} placeholder={t('autocare.requestEmailPlaceholder')} className="h-11 rounded-[var(--radius-control)] border border-border bg-background px-3 text-sm outline-none placeholder:text-muted-foreground focus-visible:ring-3 focus-visible:ring-ring/40" /></div></div>
        </section>
    )
}
