...
export function OwnerProviderCommunicationSettings({ provider, locale }: Props) {
    ...
    return <section className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm">
        ...
        <form onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-3 rounded-[var(--radius-card)] border border-border bg-background p-3 text-sm font-bold text-foreground sm:col-span-2"><input type="checkbox" checked={settings.chatEnabled} onChange={(event) => set('chatEnabled', event.target.checked)} /><MessageCircle className="size-4 text-primary" />{ru ? 'Принимаем вопросы клиентов в чате' : 'Accept customer questions in chat'}</label>
