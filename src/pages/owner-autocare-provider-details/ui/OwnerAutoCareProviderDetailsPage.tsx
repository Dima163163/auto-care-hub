...
export function OwnerAutoCareProviderDetailsPage() {
    ...
    return <main className="min-h-full bg-background px-4 py-8 lg:px-8"><section className="mx-auto max-w-6xl"><Link to={ROUTES.ownerAutoCareProviders} className="inline-flex items-center gap-2 text-sm font-black text-primary hover:underline"><ArrowLeft className="size-4" />{t('auth.accountMenuAllBranches')}</Link><PageHeader eyebrow={t('autocare.ownerProviderDetailsEyebrow')} title={provider.name} description={provider.description ?? t('common.notProvided')} /><div className="space-y-5"><ProviderOverview provider={provider} />{isDirectOwner ? <><OwnerProviderOnboardingPanel provider={provider} locale={locale} /><OwnerProviderEvidencePanel provider={provider} locale={locale} /><OwnerProviderCommunicationSettings provider={provider} /><OwnerProviderMembersPanel provider={provider} /><OwnerProviderBonusPanel provider={provider} locale={locale} /></> : <section className="rounded-[var(--radius-panel)] border border-border bg-card p-5 text-sm leading-6 text-muted-foreground shadow-sm">{locale === 'ru' ? 'Ваш доступ ограничен назначенной ролью и филиалом. Управление профилем, командой и бонусной программой доступно владельцу сервиса.' : 'Your access is limited to the assigned role and branch. Profile, team and bonus programme management are available to the service owner.'}</section>}</div></section></main>
}
