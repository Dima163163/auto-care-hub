...
function MemberList({ members, onRevoke, text }: { members: Array<{ id: string; userId: string; user: { name: string; email: string } | null; role: 'owner' | 'manager' | 'staff'; status: string }>; onRevoke: (id: string) => void; text: typeof copy.ru }) {
    return <div><h3 className="text-xs font-black uppercase tracking-[0.14em] text-muted-foreground">{text.members}</h3><div className="mt-2 space-y-2">{members.map((member) => <div key={member.id} className="flex items-center justify-between gap-3 rounded-[var(--radius-card)] border border-border p-3"><div className="flex min-w-0 items-center gap-2"><UserRound className="size-4 shrink-0 text-primary" /><div className="min-w-0"><p className="truncate text-sm font-bold text-foreground">{member.user?.name ?? member.userId}</p><p className="truncate text-xs text-muted-foreground">{member.role === 'owner' ? text.owner : member.role === 'manager' ? text.manager : text.staff} · {text.active}{member.user?.email ? ` · ${member.user.email}` : ''}</p></div></div>{member.role !== 'owner' && <button type="button" title={text.revoke} onClick={() => onRevoke(member.id)} className="rounded-[var(--radius-control)] p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><UserRoundX className="size-4" /></button>}</div>)}</div></div>
}
