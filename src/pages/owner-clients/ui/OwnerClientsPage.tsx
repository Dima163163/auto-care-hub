...
function getClients(requests: AutoCareServiceRequest[], fallbackName: string) {
    ...
    for (const request of requests) {
        ...
        const name = readSnapshotValue(request.contactSnapshot, 'name') ?? fallbackName
...
export function OwnerClientsPage() {
    ...
    const clients = getClients(query.data ?? [], t('autocare.ownerClientsDefaultName'))
    const copy: OwnerClientCopy = {
        issue: t('autocare.ownerClientsIssue'),
        noIssue: t('autocare.ownerClientsNoIssue'),
        requests: t('autocare.ownerClientsRequests'),
    }

    return <main className="min-h-full bg-background px-[var(--layout-gutter)] py-7 lg:py-10"><section className="mx-auto max-w-6xl"><PageHeader eyebrow={t('autocare.ownerClientsEyebrow')} title={t('autocare.ownerClientsTitle')} description={t('autocare.ownerClientsDescription')} />{query.isLoading && <CardsGridSkeleton label={t('common.loading')} />}{query.error && <div role="alert" className="rounded-[var(--radius-panel)] border border-destructive/30 bg-card p-6"><p className="font-semibold text-destructive">{getApiErrorMessage(query.error, t('common.failedToLoad'))}</p><RetryButton className="mt-4" onRetry={query.refetch} label={t('common.retry')} /></div>}{!query.isLoading && !query.error && (clients.length ? <div className="grid gap-4 lg:grid-cols-2">{clients.map((client) => <ClientCard key={client.id} client={client} locale={locale} copy={copy} />)}</div> : <div className="rounded-[var(--radius-panel)] border border-dashed border-border bg-card p-8 text-center text-sm text-muted-foreground">{t('autocare.ownerClientsEmpty')}</div>)}</section></main>
}

function ClientCard({ client, locale, copy }: { client: OwnerClient; locale: SupportedLocale; copy: OwnerClientCopy }) {
