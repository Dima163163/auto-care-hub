...
export function OwnerAutoCareDashboardPage() {
    ...
    return <main className="min-h-full bg-background px-[var(--layout-gutter)] py-7 lg:py-10"><section className="mx-auto max-w-6xl space-y-5"><OwnerAutoCareDashboardHero locale={locale} ownerName={user?.name} />{isLoading && <DashboardSkeleton label={t('common.loading')} />}{error && <div className="rounded-[var(--radius-panel)] border border-destructive/30 bg-card p-6"><p className="font-semibold text-destructive">{getApiErrorMessage(error, t('common.failedToLoad'))}</p><RetryButton className="mt-4" onRetry={() => void Promise.all([providers.refetch(), requests.refetch()])} label={t('common.retry')} /></div>}{!isLoading && !error && <><OwnerAutoCareMetricGrid locale={locale} metrics={metrics} /><OwnerAutoCareAnalyticsCard analytics={analytics.data} isLoading={analytics.isLoading} isError={analytics.isError} providers={providers.data ?? []} selectedProviderId={primaryProviderId} onProviderChange={setSelectedProviderId} onRetry={analytics.refetch} /><div className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]"><OwnerAutoCareRequestQueue locale={locale} requests={requests.data ?? []} /><OwnerAutoCareBranchPanel providers={providers.data ?? []} /></div><OwnerAutoCareQuickActions /><OwnerBroadcastRequestsPanel /><OwnerFleetPanel /></>}</section></main>
}
