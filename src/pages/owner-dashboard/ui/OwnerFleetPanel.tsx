...
export function OwnerFleetPanel() {
    ...
    return <section className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm">
        <div className="flex items-start gap-3">
            ...
            <div><h2 className="text-lg font-black text-foreground">{t('autocare.ownerFleetTitle')}</h2><p className="mt-1 max-w-2xl text-sm text-muted-foreground">{t('autocare.ownerFleetDescription')}</p></div>
        </div>
        <form className="mt-4 flex flex-wrap gap-2" onSubmit={(event) => void submitFleet(event)}>
            <input value={name} onChange={(event) => { setName(event.target.value); setSubmitError(null) }} placeholder={t('autocare.ownerFleetNamePlaceholder')} aria-invalid={Boolean(submitError) || undefined} aria-describedby={submitError ? 'owner-fleet-submit-error' : undefined} className="h-9 min-w-52 flex-1 rounded-[var(--radius-control)] border border-border bg-background px-3 text-xs outline-none focus-visible:ring-3 focus-visible:ring-ring/40" />
            <button type="submit" disabled={fleetState.isLoading} className="inline-flex h-9 items-center gap-1.5 rounded-[var(--radius-control)] bg-primary px-3 text-xs font-black text-primary-foreground disabled:cursor-not-allowed disabled:opacity-60"><Plus className="size-3.5" />{t('autocare.ownerFleetAdd')}</button>
        </form>
        {submitError ? <p id="owner-fleet-submit-error" role="alert" className="mt-2 text-xs font-semibold text-destructive">{getApiErrorMessage(submitError, t('autocare.ownerFleetSubmitError'))}</p> : null}
        {fleets.length ? <div className="mt-4 space-y-2">{fleets.map((fleet) => <article key={fleet.id} className="rounded-[var(--radius-card)] border border-border bg-background p-3">
            ...
            {fleet.vehicles.length ? <FleetVehicleTable vehicles={fleet.vehicles} /> : null}
            {activeFleetId === fleet.id
                ? <OwnerFleetVehicleForm fleetId={fleet.id} onCancel={() => setActiveFleetId(null)} onCreated={() => setActiveFleetId(null)} />
                : <button type="button" onClick={() => setActiveFleetId(fleet.id)} className="mt-3 text-xs font-black text-primary">{t('autocare.ownerFleetAddVehicle')}</button>}
        </article>)}</div> : null}
    </section>
}

function FleetVehicleTable({ vehicles }: { vehicles: readonly AutoCareFleetVehicle[] }) {
    const { t } = useTranslation()
    ...
    return <div
        ...
        role="region"
        tabIndex={0}
        aria-label={t('autocare.ownerFleetTableLabel')}
    >
        <table className="hidden min-w-[680px] w-full border-collapse text-left text-xs md:table">
            <thead className="bg-muted/55 text-[10px] font-black uppercase tracking-wide text-muted-foreground"><tr><th scope="col" className="px-2.5 py-2">{t('autocare.ownerFleetVehicle')}</th><th scope="col" className="px-2.5 py-2">{t('autocare.ownerFleetRegistrationNumber')}</th><th scope="col" className="px-2.5 py-2">{t('autocare.ownerFleetInternalReference')}</th><th scope="col" className="px-2.5 py-2">{t('autocare.ownerFleetVin')}</th><th scope="col" className="px-2.5 py-2">{t('autocare.ownerFleetYear')}</th></tr></thead>
        ...
        <table className="w-full border-collapse text-left text-xs md:hidden">{vehicles.map((vehicle) => <tbody key={vehicle.id} className="[&:not(:first-child)]:border-t [&:not(:first-child)]:border-border"><tr className="bg-muted/55"><th scope="row" colSpan={2} className="px-2.5 py-2 text-xs font-bold text-foreground"><span className="inline-flex items-center gap-1.5"><CarFront className="size-3.5 text-primary" />{vehicle.label}</span></th></tr><MobileVehicleField label={t('autocare.ownerFleetRegistrationNumber')} value={readSnapshotValue(vehicle.vehicleSnapshot, 'registrationNumber')} /><MobileVehicleField label={t('autocare.ownerFleetInternalReference')} value={readSnapshotValue(vehicle.vehicleSnapshot, 'internalReference')} /><MobileVehicleField label={t('autocare.ownerFleetVin')} value={readSnapshotValue(vehicle.vehicleSnapshot, 'vin')} mono /><MobileVehicleField label={t('autocare.ownerFleetYear')} value={readSnapshotValue(vehicle.vehicleSnapshot, 'year')} /></tbody>)}</table>
    </div>
}
