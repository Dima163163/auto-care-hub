...
export function OwnerFleetPanel() {
    ...
    return <section className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm">
        ...
        <form className="mt-4 flex flex-wrap gap-2" onSubmit={(event) => void submitFleet(event)}>
            <input value={name} onChange={(event) => { setName(event.target.value); setSubmitError(null) }} placeholder={copy.placeholder} aria-invalid={Boolean(submitError) || undefined} aria-describedby={submitError ? 'owner-fleet-submit-error' : undefined} className="h-9 min-w-52 flex-1 rounded-[var(--radius-control)] border border-border bg-background px-3 text-xs outline-none focus-visible:ring-3 focus-visible:ring-ring/40" />
        ...
        {submitError ? <p id="owner-fleet-submit-error" role="alert" className="mt-2 text-xs font-semibold text-destructive">{getApiErrorMessage(submitError, copy.error)}</p> : null}
...
function getCopy(locale: string) {
    return locale === 'ru'
        ? { title: 'Автопарки и партнёры', text: 'Храните автомобили компаний, чтобы согласовывать заявки и обслуживать несколько машин одним процессом.', placeholder: 'Название автопарка', add: 'Добавить автопарк', vehicle: 'Добавить автомобиль', error: 'Не удалось добавить автопарк. Попробуйте ещё раз.' }
        : { title: 'Fleet and partner vehicles', text: 'Keep company vehicles together for approvals and repeat service workflows.', placeholder: 'Fleet name', add: 'Add fleet', vehicle: 'Add vehicle', error: 'Could not add the fleet. Please try again.' }
}
