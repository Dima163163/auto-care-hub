...
export function OwnerBroadcastRequestsPanel() {
    ...
    const submit = async (requestId: string, serviceDefinitionId: string) => {
        ...
        try {
            ...
            setSubmitError(getApiErrorMessage(error, t('autocare.ownerBroadcastSubmitError')))
        }
    }
    return <section className="rounded-[var(--radius-panel)] border border-border bg-card p-5 shadow-sm"><div className="flex items-start gap-3"><span className="flex size-10 items-center justify-center rounded-[var(--radius-control)] bg-primary/10 text-primary"><MessageSquareQuote className="size-5" /></span><div><h2 className="text-lg font-black text-foreground">{t('autocare.ownerBroadcastTitle')}</h2><p className="mt-1 text-sm text-muted-foreground">{t('autocare.ownerBroadcastDescription')}</p></div></div>{requests.length === 0 ? <p className="mt-4 rounded-[var(--radius-control)] bg-background p-3 text-xs font-semibold text-muted-foreground">{t('autocare.ownerBroadcastEmpty')}</p> : <div className="mt-4 space-y-3">{requests.slice(0, 4).map((request) => <article key={request.id} className="rounded-[var(--radius-card)] border border-border bg-background p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-foreground">{request.serviceSlug}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{request.issueDescription}</p></div><span className="rounded-full bg-status-success-surface px-2 py-1 text-[10px] font-bold text-status-success-foreground">{request.offers.length}</span></div>{activeId === request.id ? <div className="mt-3 flex gap-2"><input value={amount} onChange={(event) => { setAmount(event.target.value); setSubmitError(null) }} inputMode="decimal" placeholder={t('autocare.ownerBroadcastAmountPlaceholder')} aria-invalid={Boolean(submitError) || undefined} aria-describedby={submitError ? 'owner-broadcast-offer-error' : undefined} className="h-9 min-w-0 flex-1 rounded-[var(--radius-control)] border border-border bg-card px-3 text-xs outline-none focus-visible:ring-3 focus-visible:ring-ring/40" /><button type="button" disabled={offerState.isLoading} onClick={() => void submit(request.id, request.serviceDefinitionId)} className="inline-flex h-9 items-center gap-1.5 rounded-[var(--radius-control)] bg-primary px-3 text-xs font-black text-primary-foreground disabled:opacity-50"><Send className="size-3.5" />{t('autocare.ownerBroadcastSend')}</button></div> : <button type="button" onClick={() => { setActiveId(request.id); setSubmitError(null) }} className="mt-3 text-xs font-black text-primary">{t('autocare.ownerBroadcastSend')}</button>}</article>)}</div>}{offerState.isSuccess ? <p className="mt-3 text-xs font-bold text-status-success-foreground">{t('autocare.ownerBroadcastSent')}</p> : null}{submitError ? <p id="owner-broadcast-offer-error" role="alert" className="mt-3 text-xs font-semibold text-destructive">{submitError}</p> : null}</section>
}
