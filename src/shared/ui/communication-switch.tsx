...
export function CommunicationSwitch({ id, checked, onChange, label, description, disabled = false, compact = false, inline = false, hideLabel = false, testId }: CommunicationSwitchProps) {
    return <label htmlFor={id} className={`${inline ? 'inline-flex w-auto shrink-0' : 'flex'} cursor-pointer items-center justify-between gap-3 rounded-[var(--radius-card)] border border-border bg-background ${compact ? 'px-3 py-2' : 'p-3'}`}>
        <span className={hideLabel ? 'sr-only' : 'min-w-0'}><span className="block text-xs font-black text-foreground">{label}</span>{description && <span className="mt-0.5 block text-[11px] leading-4 font-medium text-muted-foreground">{description}</span>}</span>
