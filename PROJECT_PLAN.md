...
| Provider profile `/services/:id` | Public profile API, approved hero/gallery layout, service offers, amenities, map, live availability, public approved review aggregates/media and authenticated provider favorites are implemented. Contact data now preserves all provider phone numbers. | Connect service inquiries to the profile CTA, finish provider-owned gallery moderation and replace remaining demo fallbacks. |
