...
test('capacity contract keeps the owner screen compact', () => {
    ...
    if (!calendarSource.includes('autocare.ownerCapacityCalendarTitle')) throw new Error('compact calendar title translation is missing')
    if (!calendarSource.includes('autocare.ownerCapacityCalendarConfirmedCount')) throw new Error('branch appointment summary translation is missing')
...
test('capacity contract keeps detailed resources isolated for post-MVP', () => {
    if (!resourcesSource.includes('autocare.ownerCapacityResourcesEmpty')) throw new Error('empty resource translation is not guarded')
