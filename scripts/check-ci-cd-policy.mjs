...
export function validateCiCdPolicy({ quality, promotion, protection, readme }) {
    const checks = [
        ...
        ['Promotion waits for the completed Quality workflow', /workflow_run:/s.test(promotion) && /- Quality/.test(promotion) && /- completed/.test(promotion)],
        ['Promotion is limited to the trusted dev branch', /head_branch == 'dev'/.test(promotion) && /head_repository\.full_name == github\.repository/.test(promotion)],
