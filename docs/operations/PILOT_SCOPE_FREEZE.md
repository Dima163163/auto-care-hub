...
Исполняемая SDD-спецификация для сценариев даты, availability и quote:
[MVP_BOOKING_QUOTE_SPEC.md](../specs/MVP_BOOKING_QUOTE_SPEC.md). Она уточняет
проверяемое поведение существующих условий и не добавляет новых единиц в
знаменатель.

## SDD-порядок для следующих изменений

Каждая функциональная порция проходит один и тот же короткий цикл:

1. **Spec** — фиксируем границы, нормативные требования и out-of-scope до
   изменения кода.
2. **Examples** — добавляем наблюдаемые сценарии Given/When/Then для happy path,
   отказов и повторов.
3. **Implementation** — меняем только код, необходимый для этих сценариев.
4. **Evidence** — запускаем unit/API/browser checks и записываем фактический
   результат с датой и командой.
5. **Review** — сверяем регрессию, безопасность и scope; только доказанный ID
   меняет накопительный статус. Новая идея без обязательного acceptance
   остаётся в POST/EXT-реестре и не меняет знаменатель.

Spec и acceptance evidence — единая связка: тест без требования не становится
новой обязательной функцией, а требование без проверяемого сценария не считается
готовым. Внешние staging, security и real-pilot gates по-прежнему требуют
отдельного evidence и не заменяются локальными mock-тестами.
