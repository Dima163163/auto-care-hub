...
- Owner request queue and request list now use typed queue labels, locale-aware
  service-label fallback and shared date/currency formatters; sorting, filtering
  and request selection semantics remain unchanged.
...
- Post-provider-change `npm run check:local-mvp` passes all 43 checks, including
  ...
  **1 file / 1 test PASS**, including the required-note `role="alert"` path.
- Admin appeals panel now uses typed translation keys and shared locale date
  formatting for appeal/status/subject filters, evidence and decision states;
  appeal query and mutation semantics remain unchanged.
...
- Admin chat reports now use typed translation keys and shared locale date
  formatting for report/status/category labels, fallbacks and decision controls;
  query filter and moderation mutation semantics remain unchanged.
...
- The deployment probe also exposed a source-config mismatch: the canonical
  `NEXT_PUBLIC_SITE_URL`/sitemap host was absent from Render `CORS_ORIGINS` and
  was not the `FRONTEND_ORIGIN`. `render.yaml` now includes the canonical host
